...
3. **The `events` contract is frozen.** PR #137 ratified the cumulative Phase 1 schema and `docs/EVENT-REVISION-CONTRACT.md` under its recorded approval exception. Every later shared/core-table change requires the approvals in `docs/DOCUMENTATION-GOVERNANCE.md` §6; never add a column in a feature branch without them.
