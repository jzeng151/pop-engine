...
2. **Never edit verification statuses.** The `verification` blocks in the rules file (SOURCE_CONFIRMED / OFFICIAL_CONFLICT / RESEARCH_REQUIRED / VERIFIED) are changed by the product owner after checking a primary source, and a status the product owner authored still needs the second signatory `docs/DOCUMENTATION-GOVERNANCE.md` §6's closing paragraph requires: a verification status is regulatory publication, and no person approves their own alone. Not by you, not by your AI.
