...
function dateChangeCopy(notice: MovedDeadlineNotice): string | null {
  ...
  switch (change.kind) {
    ...
    case "now_computed":
      return (
        `PopEngine now computes a filing deadline of ${change.current} for this requirement.`
      );
  }
}
...
function stateChangeCopy(notice: MovedDeadlineNotice): string | null {
  ...
  const gate =
    change.previous.gated !== change.current.gated
      ...
      : "";
  return (
    `What applied before about this deadline: ${previous}. What applies now: ${current}.${gate}`
  );
}
