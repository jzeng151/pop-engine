...
describe.runIf(databaseUrl.length > 0)("migration 001 and rules sync", () => {
  ...
  it("creates every approved table and mirrors the intake registry", async () => {
    ...
    expect(tables.rows.map(({ table_name }) => table_name)).toEqual(
      [
        // F-203: that an alert was handed to a provider, written before the handoff so a crash
        // between provider-accept and COMMIT is not read as an alert nobody ever tried
        // (migration 014).
        "alert_send_attempts",
        "alerts",
        "checkins",
