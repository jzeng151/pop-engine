...
7. **Verdict model:** the four-state verdict stays as the top-level summary. It uses per-finding deadline statuses (ON_TRACK / DEADLINE_APPROACHING / PUBLISHED_DEADLINE_MISSED / NOT_CALCULABLE / NOT_APPLICABLE), and a resolved prohibition or ineligibility also drives INFEASIBLE without needing a deadline. INFEASIBLE copy = "Blocked as scoped" and distinguishes that blocker from a missed published deadline. The 14-day slack threshold is labeled as internal policy.
...
- **Dev 1 — Rules engine + verdict:** F-201, F-102; owns engine fidelity to `rules/nyc-rules.v2.13.json` and the fixture suite. Verify: full fixture suite (scenarios + boundaries) passes as automated tests.
...
- `rules/nyc-rules.v2.13.json` is the crown jewel; version it like code. No fact enters it without an evidence reference to `VERIFICATION-SOURCES.md`; gaps are RESEARCH_REQUIRED, conflicts are OFFICIAL_CONFLICT, never guesses.
