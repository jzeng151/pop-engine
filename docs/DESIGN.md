...
5. **Ruleset baseline is the corrected subset `nyc.v2.10`** (42 rules + 4 advisories, evidence-linked to `VERIFICATION-SOURCES.md`). The 59-rule draft stays in `rules/proposals/` as the post-capstone target. Scenario fixtures v7 derive from the ruleset.
...
One integration point (the `events` schema — ratified 2026-07-26 through PR #137; later shared/core-table changes remain team decisions); four lanes with minimal merge conflicts:
...
- `rules/nyc-rules.v2.10.json` is the crown jewel; version it like code. No fact enters it without an evidence reference to `VERIFICATION-SOURCES.md`; gaps are RESEARCH_REQUIRED, conflicts are OFFICIAL_CONFLICT, never guesses.
