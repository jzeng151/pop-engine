...
**Organizer-summary amendment (2026-07-29):** retargeted to nyc.v2.11, which adds source-linked plain-language summaries without changing any regulatory trigger, deadline, fee, portal, verification status, finding, or verdict.
**Parks threshold amendment (2026-08-05):** §1's lead-time bullet states the Parks special event permit as required above 20 guests, and states that at exactly 20 a permit may be required because the published sources disagree there, replacing "20+ guests", which included 20. Amended under `docs/DOCUMENTATION-GOVERNANCE.md` §2 against `rules/nyc-rules.v2.11.json`, which publishes `PARKS-EVENT-001` above 20 at `SOURCE_CONFIRMED` and `PARKS-EVENT-EXACTLY-20-001` at exactly 20 at `OFFICIAL_CONFLICT`, the Parks FAQ's "twenty or more people" against the portal's and NYC311's "more than 20". Because the sentence states a trigger boundary and a verification status, it is separately approved under §6's "Regulatory source/status/content" row by the product owner. §6's closing paragraph required a second signatory for a regulatory publication, and the product owner retired that requirement repo-wide on 2026-08-05 (recorded in `docs/BASELINE.md` as "Decision 2026-08-05 (product owner, second-party review retired)", landing on PR #248), so no second signatory is required here and none is claimed. No rule, trigger, threshold, deadline, fee, portal, verification status, finding, or verdict changes. Recorded in `docs/BASELINE.md`.
**Companion docs:** `BASELINE.md` (current artifact versions) · `ROADMAP.md` (phases + features) · `DESIGN.md` (lanes, gates, demo plan) · `ARCHITECTURE.md` (technical design) · `ARCHITECTURE-FUTURE.md` (Phase 2+ target) · `test-scenario-answer-key.md` (scenario fixtures v7) · `rules/nyc-rules.v2.11.json` (published ruleset).
**Permit facts:** every permit fact traces to `rules/nyc-rules.v2.11.json`, whose facts carry evidence references to fetch-confirmed quotes in `VERIFICATION-SOURCES.md`. Verification statuses (SOURCE_CONFIRMED / OFFICIAL_CONFLICT / RESEARCH_REQUIRED / COVERAGE_GAP) render honestly in-product. No permit fact is ever invented.
...
| :--------------------------------------------- | :------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------- |
| Complete plan generation                       | Demo events produce correct plans     | Expected findings matched vs. the approved fixture suite (6 scenarios + boundary fixtures, derived from ruleset nyc.v2.11)                         | 100% of expected findings, 0 false omissions, 0 false additions |
...
- System evaluates the published ruleset (`rules/nyc-rules.v2.11.json`: 42 rules + 4 advisories) against the intake and returns every applicable finding: kind (permit / insurance / notification / registration / eligibility / prohibition / advisory / note), disposition (required / may-be-required / prohibited-or-ineligible / advisory / no-new-requirement), name, agency, typed deadline, fee, portal, and the rule + triggering answers that produced it.
...
- "Rules snapshot nyc.v2.11 · published July 29, 2026" renders on every plan. Never "verified as of": a snapshot date means published-on, not all-facts-verified-on.
