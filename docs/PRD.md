...
**Parks threshold amendment (2026-08-05):** §1's lead-time bullet states the Parks special event permit as required above 20 guests, and states that at exactly 20 a permit may be required because the published sources disagree there, replacing "20+ guests", which included 20. Amended under `docs/DOCUMENTATION-GOVERNANCE.md` §2 against `rules/nyc-rules.v2.11.json`, which publishes `PARKS-EVENT-001` above 20 at `SOURCE_CONFIRMED` and `PARKS-EVENT-EXACTLY-20-001` at exactly 20 at `OFFICIAL_CONFLICT`, the Parks FAQ's "twenty or more people" against the portal's and NYC311's "more than 20". Because the sentence states a trigger boundary and a verification status, it is separately approved under §6's "Regulatory source/status/content" row by the product owner. No rule, trigger, threshold, deadline, fee, portal, verification status, finding, or verdict changes. Recorded in `docs/BASELINE.md`.
