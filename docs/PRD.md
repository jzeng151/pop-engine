...
**Companion docs:** `BASELINE.md` (current artifact versions) · `ROADMAP.md` (phases + features) · `DESIGN.md` (lanes, gates, demo plan) · `ARCHITECTURE.md` (technical design) · `ARCHITECTURE-FUTURE.md` (Phase 2+ target) · `test-scenario-answer-key.md` (scenario fixtures v6) · `rules/nyc-rules.v2.8.json` (published ruleset).
...
The seven features below must be complete, real, and demoable. No mocks in this path. Acceptance detail lives in `/specs`; the scenario + boundary fixtures in `test-scenario-answer-key.md` (v6, derived from ruleset nyc.v2.8) are the acceptance suite for F-201/F-102.
...
- CONDITIONAL renders when material unknowns change the outcome, with every branch evaluated and shown (Scenario F: license coverage and sound audibility). Scenario F also records the coarse assembly-approval answer as confirmation context, but it cannot establish whether the current PACO and PA permit cover the exact event space, use, occupancy, and layout (#188).
