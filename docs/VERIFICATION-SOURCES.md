...
**Purpose:** Primary-source research record for the regulatory items in `OPEN-QUESTIONS.md` §2, begun 2026-07-22. **Nothing in this document is a verification.** Only the product owner may promote a fact in a new immutable rules publication, per the process note, and that approval is the whole requirement under `DOCUMENTATION-GOVERNANCE.md` §6 even where the product owner authored the promotion. SUPPORT / CONTRADICT / NOT ADDRESS labels are researchers' candidate assessments of fetched text against the encoded claim, for triage only.
