...
**nyc.v2.12 retarget (2026-08-12, product owner, issues #258 and #287):** `DOB-PROP-TRUSS-001` now resolves no/unknown/yes height to no finding/MAY_BE_REQUIRED/REQUIRED; `DOB-TALL-STRUCTURE-001` keeps MAY_BE_REQUIRED with consistently hedged organizer copy; and `FDNY-FUEL-001` now covers charcoal/wood at any venue or propane outside parks. Scenario D's charcoal finding and all six scenario verdicts remain unchanged. The boundary suite adds the prop/truss tri-state and four fuel/location combinations; a resolved Parks propane prohibition now drives overall INFEASIBLE under F-102.

**nyc.v2.13 retarget (2026-08-12, product owner, issue #144):** the two source-less `COVERAGE_GAP` advisories retain their triggers, status, finding shape, and expected presence while removing embedded alcohol-prohibition and out-of-scope SAPO-deadline claims. Their copy now says only that the combination is not modeled and directs confirmation with the relevant agency. No scenario finding, deadline, verification status, or verdict changes.
...
| INFEASIBLE       | a resolved non-conflict route publishes `prohibited_or_ineligible`, or a resolved route at or above `required` has a PUBLISHED_DEADLINE_MISSED status | “Blocked as scoped”; detail identifies the prohibition/ineligibility or missed published deadline |

**INFEASIBLE row amended 2026-08-08 (product owner), and no fixture expectation moves.** It read "a definitively-required finding", which is narrower than the engine: `prohibited_or_ineligible` is stronger than `required` and is not a requirement at all, so a finding that was both barred and past its published filing window rendered CONDITIONAL. The clause about the trigger having resolved is the invariant that keeps the wider bar safe, not a new condition: an unknown-conditioned finding never renders INFEASIBLE. `specs/F-102-feasibility-verdict.md` Acceptance Criterion 10 and `docs/ARCHITECTURE.md`'s verdict algorithm step 4 carry the same rule, and `docs/BASELINE.md` records the decision. Every scenario and boundary fixture in this document keeps its expected findings, statuses and verdict: the only two published rules that reach `prohibited_or_ineligible`, `SAPO-BLOCK-PARTY-ELIG-001` and `PARKS-PROPANE-001`, publish no `deadline` and carry no `dedupe_key`, so neither can be missed and neither can acquire a filing window from a group.

**INFEASIBLE row amended 2026-08-12 (product owner, issue #258).** A resolved prohibition or ineligibility now blocks independently of deadline status. The six named scenarios keep their verdicts; the block-party-plus-sales boundary fixture now expressly expects INFEASIBLE. Unresolved and OFFICIAL_CONFLICT prohibitions remain non-blocking by themselves.
...
2. Alcohol branch on venue_license_covers_event_area:
   ...
   - no → SLA-ONEDAY-001: 15 business days required; **under the supplied fixture calendar, only 14 business days remain to 2026-08-11** (no holidays in the window) → PUBLISHED_DEADLINE_MISSED on this branch; SLA-CATERING-001 same window (and requires real food + a currently licensed caterer)

   Production returns NOT_CALCULABLE while the pinned calendar has no published holiday list.
...
**Test purpose:** the hardest behavior: verdicts hinging on multiple user-confirmable facts, with deterministic business-day math against a supplied calendar and an explicit production fallback when that calendar is unavailable.
...
- Block party + selling_anything → PROHIBITED_OR_INELIGIBLE and overall INFEASIBLE; block party + ride → insurance finding appears.
- tent_area_sqft 401 → DOB-TENT-001 REQUIRED; 400 → CONDITIONAL; 399 → nothing (absent other triggers).
- prop_truss with structure_over_10ft_tall yes → DOB-PROP-TRUSS-001 REQUIRED; unknown → CONDITIONAL; no → nothing.
- park propane only → PARKS-PROPANE-001 and no FDNY-FUEL-001; park charcoal/wood → FDNY-FUEL-001; mixed park propane + charcoal/wood → both findings, with the FDNY route concerning charcoal/wood; non-park propane → FDNY-FUEL-001.
...
- sapo_event_type=other_sapo_class → assertion-free ADV-SAPO-OTHER-CLASS-001 coverage advisory; no permit type, deadline, fee, portal, exception, source, or agency is asserted.
- obstructs_public_way=no on a sidewalk → SAPO-SCOPE-001 no-new-requirement note.
- Every negative-answer boundary retains its substantive result and also emits the applicable nyc.v2.13 confirmation; the acceptance suite compares substantive rule IDs separately from confirmation IDs so neither can hide a regression in the other.
