...
**nyc.v2.10 retarget (2026-07-29, issue #181):** SAPO-BLOCK-PARTY-ELIG-001's citation now attributes the published prohibition only to the CECM block-parties page. This is citation metadata only: no fixture input, expected finding, verification status, deadline, count, or verdict changes, so the fixture document remains v7.
...
- Every negative-answer boundary retains its substantive result and also emits the applicable nyc.v2.10 confirmation; the acceptance suite compares substantive rule IDs separately from confirmation IDs so neither can hide a regression in the other.
