...
**Second-party review under a one-person team (2026-08-05).** Requirements that some other party review, approve, verify, witness or sign something are one class, not a series of unrelated lines, and the 2026-08-04 collapse reaches them differently depending on what they rest on. Where the requirement is that a regulatory publication or a verification status not be approved by its author alone, it rests on the closing paragraph's first sentence, which is unconditional; the collapse does not reach it, every route that reaches one carries it in its own words, and it stays as written. Where the requirement is only that an approval name a capacity, the collapse answers it: the capacity is the product owner. What is left is the group that rests on the closing paragraph's second sentence alone, because it asks for a distinct reviewer of something that publishes no ruleset and asserts no regulatory fact: the spec-approval reviewer in `specs/F-103-scope-comparator.md`, `specs/F-107-save-resume.md`, `specs/F-306-waitlist.md`, `specs/F-405-day-of-runbook.md` and `specs/F-502-historical-event-comparison.md`; AD-15's coordinated review in `docs/ARCHITECTURE-FUTURE.md`, and there only for the shared contracts that assert no regulatory fact, an executable fixture or expected output being a regulatory publication that keeps the first sentence's second signatory; and the gate that is not an approval recorded in a document at all, `CONTRIBUTING.md`'s "one teammate's review. You cannot merge your own PR unreviewed" and the matching PR-checklist line. A team of one cannot make the author and the reviewer distinct, so those requirements are **UNMET**. They are recorded as unmet rather than deleted or read as satisfied. What would satisfy each is the same thing: a second contributor with repository access who reviews the artifact before it is approved, or the pull request before it merges, and is named on it beside the author.

Unmet is not satisfied, and it is not permission. **Nothing in this paragraph approves anything, and no artifact becomes approved because the reviewer it required does not exist.** The five specs above stay `PROPOSED` with an approval date of `—` and their Approval Blockers open; §3 governs them unchanged, so they are not implementable, and no one may read the missing reviewer as author approval or as a reason to proceed. The PR gate is unmet in the same direction: a PR that carries a regulatory publication or a verification-status change is reached by the closing paragraph's first sentence independently of `CONTRIBUTING.md`, so it still requires the second signatory and an unreviewed self-merge does not release it. Each of these lines says on its own face that its second-party element is unmet, so a reader does not have to infer it, and each says what remains outstanding rather than declaring it done.
