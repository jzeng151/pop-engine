...
- Inputs are an exact event revision and lifecycle generation, channel, approved template content, and one Roadmap offset; outputs are a recipient snapshot plus message jobs/attempts pinned to that revision, lifecycle generation, and exact channel-endpoint version.
