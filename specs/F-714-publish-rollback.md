...
- F-710/F-711/F-712/F-713 contracts, F-703 separate rules-admin roles, and `DOCUMENTATION-GOVERNANCE.md` §6 approvals. F-701 supplies the authenticated actor whose current platform authority `F714-AC-09` and `F714-AC-10` re-read; F-701 and F-703 are both PROPOSED, so that gate is not an approved input today and this spec is not implementable against it until they are approved and listed in `docs/BASELINE.md`.
...
10. **F714-AC-10:** Every read this feature defines is a rules-admin function admitted only by the separate F-703 platform rules-admin authority, on the terms `F703-AC-04` states for every such function, with the acting actor's current platform authority re-read server-side at each request rather than taken from the session, a client-supplied role claim, or the authority held when the flow began. That covers the record and status reads the System Impact API row names, the confirmation-screen read of the candidate, current and new version and checksum, changelog, approvals, and full-suite result under UI and Accessibility, and the response path of a recognized `F714-AC-03` retry: returning that request's original immutable result is a read of the publish or rollback record, so the responder re-reads the actor's current platform authority at that read, while AC-03's exemption from the fresh-authentication proof stands exactly as `F714-AC-09` states it, because the retry moves no pointer. `F714-AC-09` remains the rule for the two pointer-moving mutations and is not restated here. Workspace membership admits nothing on any of these surfaces, and a refused request returns a response that does not distinguish a missing artifact, candidate, record, or status from one the actor may not see.

    Without this criterion the mutations are gated and the reads are not. `F714-AC-09` attaches its re-read and proof to the transaction that advances a pointer, and AC-01 through AC-08 compare checksums, approvals, F-712 results, lineage, and pointers without asking who receives the answer, so the confirmation screen hands a candidate's changelog, approvals, and version lineage, and the record and status surfaces hand the full publish and rollback history, to any authenticated caller, and a recognized retry returns the immutable result of the most privileged write in the system to whoever presents the committed identity, including an actor whose platform authority was revoked after the original request.

    One input this criterion needs is not established by any approved artifact today and is not invented here. F-703 is PROPOSED and names no role set, so the platform authority above cannot be named beyond its separateness from workspace roles. Until F-703 is approved this criterion is testable only as "every publish and rollback record, status, and confirmation read, including the recognized-retry response, is refused unless the acting actor's current platform rules-admin authority is confirmed server-side at that read, and a refusal discloses nothing about which artifacts, candidates, or records exist", not against a named role or permission identifier. Naming the publish and rollback read authority with F-703 is an approval blocker below.
...
- F714-AC-10 includes a fixture in which an authenticated actor holding no platform rules-admin role, including a workspace owner, is refused the record and status reads and the confirmation-screen read, with a response indistinguishable from the one for records that do not exist, and a fixture in which platform authority is revoked after a publication commits and a recognized `F714-AC-03` retry presenting the committed identity is refused the original result, with no pointer or record disturbed.
...
- Approve F-701 and F-703, and name with F-703 the publish and rollback record, status, and confirmation read authority `F714-AC-10` checks. That criterion checks a platform authority no approved artifact defines today and may not invent one, so until the matrix names it the criterion is testable only at the check-shape level stated there.
