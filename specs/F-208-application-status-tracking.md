...
8. **F208-AC-08:** The checklist requirement, the immutable finding it links to, the plan or checklist generation the creation names, the Event they belong to, and the application being created must all resolve to the same workspace, compared server-side inside the transaction that commits the creation, and every later recording, correction, and read holds the same records to the workspace that owns the application it acts on. F208-AC-01 requires the creation to name a requirement, its immutable finding, and the exact generation it read them from, and it enforces the supported-count slot and the generation compare-and-swap against those records wherever they live; F208-AC-07 admits the operation by the acting actor's current membership of the workspace that owns the application. A user holding an active membership of workspace A and an active membership of workspace B satisfies every one of those while creating an application in B against a checklist requirement, finding, and generation from A: the actor is legitimately authorized in B, the slot key over A's requirement and finding is unique because B has no such record, and the generation genuinely is current in A. A mismatch rejects the creation in full, creates no application, claims no slot, appends no history and no projection, and returns a response that does not distinguish a requirement, finding, generation, Event, or application that does not exist from one that resolves to a different workspace, per F208-AC-05.

   The harm is that B receives a plan-backed application whose provenance is A's private regulatory finding, so B's organizer reads a correctly provenanced permit record sourced from a plan they may not open, and the record is not the user-recorded, source-provenanced unexpected-requirement kind F208-AC-01 requires for an application absent from the plan; it presents as plan-derived because the link is real. It is invisible from A, because being linked and cited changes nothing there. F208-AC-07's own explanation covers the caller who holds no membership near these records; this is the caller who holds one on each side.

   This applies `F702-AC-10`'s fourth obligation, which states once for the whole feature set that every record an operation names must resolve to one workspace, to the records this creation names together. `F604-AC-09` states the same rule for the F-602 proposal, source document, and F-208 or F-209 current record a reconciliation pairs, and `F214-AC-09` states it for the F-208, F-209, and F-210 evidence versions a vendor projection pins; this is that rule applied to the plan-side records an application creation names, and it is stated here because an implementation is built to this spec's criteria. No approved artifact establishes cross-workspace plan or finding sharing and this criterion does not invent it; admitting one would be a Scope change returning to the PRD and Roadmap decision process under `docs/DOCUMENTATION-GOVERNANCE.md` §7. F-702 is `PROPOSED`, so the workspace a record resolves to is not an approved input today and this criterion is not implementable until F-702 is approved and listed in `docs/BASELINE.md`; until then it is testable only as "a creation whose checklist requirement, immutable finding, plan generation, Event, and new application do not all resolve to one workspace is refused, creates nothing, and discloses neither side", not against a named workspace model.
...
- F208-AC-08 includes a fixture in which an actor holding an active membership of two workspaces creates an application in one while naming a checklist requirement, immutable finding, and plan generation from the other and is refused in full with no application, slot, or history written and a response that does not distinguish absence from a different workspace, a fixture proving the same refusal when only the generation belongs to the other workspace, and a same-workspace control fixture in which the identical creation commits.
...
- Approve F-702 so that the workspace a checklist requirement, immutable finding, plan generation, Event, and application each resolve to is an approved input `F208-AC-08` can compare. That criterion compares a boundary no approved artifact defines today and may not invent one.
