...
8. **F209-AC-08:** Fee, estimate, invoice, payment, document-state, and final-permit mutations bind a stable request identity to the original immutable result; a lost-response retry returns that result without duplicating history or rollups. Each mutation compare-and-swaps the expected ledger version and every affected entry version; a mismatch rejects the whole mutation without changing history, projections, or rollups.
