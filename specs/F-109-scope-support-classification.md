...
4. **F109-AC-04:** Scope support is carried separately from `ARCHITECTURE-FUTURE.md` §7.1 result completeness, feasibility verdict, rule verification status, finding disposition, and evaluation failure, as ADR AD-20 decides. A material `COVERAGE_GAP` forces `OUTSIDE_VALIDATED_COVERAGE`; partial or unsupported scope cannot produce a complete-plan label; ambiguous or awaiting-information scope cannot produce one until resolved. Result completeness owns that label.
...
- Carry the T-4 resolution recorded as ADR AD-20 without collapsing the three classifications or redefining `COVERAGE_GAP`.
