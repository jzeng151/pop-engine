...
**Phase:** 1 (core, week 1) · **Lane:** Dev 2 · **Depends on:** events schema (Phase 0), ruleset nyc.v2.13 ratified (BASELINE.md) · **Feeds:** everything (single source of truth)
**Updated:** 2026-07-22 against nyc.v2.1; retargeted through nyc.v2.8 for the changes recorded in `docs/BASELINE.md`, to nyc.v2.9 on 2026-07-29, to nyc.v2.10 the same day for issue #181's citation-only correction, to nyc.v2.11 for organizer summaries, to nyc.v2.12 on 2026-08-12 for issues #258 and #287, and to nyc.v2.13 the same day for issue #144's assertion-free coverage-advisory publication. The shared issue #178 publication adds the nine issue #107 named confirmations, replaces the coarse assembly-approval question with F-110's two document-specific tri-states under the same private-venue/headcount gate, and removes the issue #194 food-exception claim from active intake. The superseded database columns remain historical storage; no rule or verdict reinterprets them. The v2.10 and v2.11 retargets change no field, trigger, finding, status, or verdict; v2.12 changes existing structure and fuel triggers without changing the intake registry. Amended 2026-08-10 by product-owner direction in issue #272 to require one client-side pass over every currently visible required answer before a save request, and in issue #273 to generate the first plan immediately after creating an event. Amended 2026-08-11 by the product owner's approval of option 1 in SPEC-CONFLICT #281 to make initial creation durably replayable after an unobserved response, and by the product owner's PR #277 review direction to make the keyed first-plan operation converge under the event lock.
...
The field list, enums, and asked-when conditions come from the ruleset's `intake_fields` registry (`rules/nyc-rules.v2.13.json`) — **the registry is authoritative; do not duplicate or drift from it.** Field groups (mirrored by the `events` table in ARCHITECTURE.md):
...
- `POST /api/events` + UUID `Idempotency-Key` → a newly created event row (`201`, `revision_counter = 1) or the original row for a recognized same-body replay (`200`); per-field validation errors. Missing or malformed keys return 400. Reusing a committed key with a different JSON body returns 409 and writes nothing.
- `POST /api/events/:id/plan` + the event's create `Idempotency-Key` → the first plan (`201`) or that same stored plan for a recognized retry (`200`). A malformed key returns 400 and a key from another event returns 409. Unkeyed regeneration keeps creating a new immutable plan.
...
9. Before creating or updating an event, the client reports every unanswered required field that is currently visible under the registry's `asked_when` conditions in one summary, links each item to its control, and focuses (therefore scrolls to) the first. Correcting a field clears that field's stale error immediately. Hidden conditional fields are not required until they are asked. Server validation remains authoritative.
10. After a successful initial `POST /api/events`, the client calls `POST /api/events/:id/plan` with the same `Idempotency-Key` and navigates directly to `/events/:id/plan` when generation succeeds. Editing an existing event keeps the overview redirect in Acceptance Criterion 8 and verifies that a plan exists before offering promotion. If initial generation fails, the saved event remains available and the form reports that the event was saved without a generated plan, with the existing plan link providing the manual generation path.
11. The first accepted create atomically stores its UUID `Idempotency-Key` and exact JSON intake body with the event. A same-key, same-body replay returns that original event before current intake validation, including after the original event date has become past; concurrent copies create one event. A same-key, different-body replay returns 409 and writes nothing. Missing or malformed keys return 400 and write nothing. After a create outcome the browser did not observe, it retains the original key, body, and answers in per-tab session storage across a form remount or reload and retries that immutable operation before applying the current client's validation or starting another create. Initial plan attempts reuse that key and serialize on the event row: the first inserts a plan and a concurrent or later retry returns it instead of inserting another. Unkeyed manual regeneration remains a new immutable plan. Recovery remains stored until initial planning has a known stored-or-not-stored outcome, then Acceptance Criterion 10 continues from the recovered event ID. Historical rows may carry neither replay column; every newly created row carries both. Because old and new web/api builds are mutually incompatible for this request, the migration-015 release keeps organizer traffic closed until both deploys and an end-to-end synthetic create check complete (`DEPLOY.md`).
...
- Create or first-plan responses are lost: Save again replays the original key and body, recovers the same event ID, and repeats the keyed first-plan operation; concurrent copies converge on one stored first plan.
