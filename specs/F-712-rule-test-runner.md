...
3. **F712-AC-03:** Failure, evaluation error, timeout, cancellation, missing fixture, or changed input cannot count as a passing publication gate.
   ...
   | R-03 | Cancel before the run starts                         | an authorized platform administrator | `queued`             | `cancelled`                                                                       | the cancellation is refused and reports `running`; the administrator cancels again against `running` under R-04; the cancellation commits its own stable client-supplied request identity under a uniqueness constraint scoped to the run |
   | R-04 | Cancel after the run starts                          | an authorized platform administrator | `running`            | `cancelled`                                                                       | the cancellation is refused and reports the terminal result that committed first; it overwrites nothing; the cancellation commits its own stable client-supplied request identity under a uniqueness constraint scoped to the run         |
   ...
   | R-07 | Cancel a run that is already terminal                | nobody; the request is rejected      | terminal             | none                                                                              | the version is read, compared, and rejected before any write; the run keeps the state it reached, and this row is reached only after R-08 has found no committed cancellation identity for the request                                    |
   | R-08 | Cancel replayed on a committed cancellation identity | the same actor as R-03 or R-04       | terminal `cancelled` | none; the outcome that cancellation recorded is returned                          | no version is compared, because the commit being replayed is what made the state terminal; the identity is resolved first, so R-07 is never reached for a recognized replay                                                               |
   ...
   R-08 exists because R-03, R-04, and R-07 otherwise disagree about one request. F712-AC-06 gives run creation a stable identity and stops there, so cancellation is the one administrator transition on a run that already exists with nothing to resolve against. When a cancellation commits and its response is lost, the retry presents the same intent against a run whose state its own commit made terminal, so R-07 rejects it and reports the run as already terminal rather than returning the cancellation that in fact succeeded. The administrator is told their cancellation was rejected by the operation that performed it, and the recoveries left are re-running the candidate or re-checking the publication gate, both of which spend isolated runner capacity for a run that is already cancelled. Each cancellation therefore commits its own stable client-supplied request identity under a uniqueness constraint scoped to the run, distinct from F712-AC-06's creation identity, which is scoped to the candidate and covers only the transition that brings a run into existence. That identity binds its operands under the rule `F411-AC-08` states once for every client-supplied identity on this branch, which here are the run and the state and version the request named, so a reuse naming a different run is refused as a conflict and cancels nothing. It is resolved in the order `F411-AC-08` states once for every client-supplied identity on this branch, applied here rather than restated: the identity is looked up before the state and version comparison this table requires, so a recognized replay returns the cancellation's recorded outcome under R-08 and never reaches R-07, while a request whose identity resolves to no committed outcome is held to R-03, R-04, or R-07 on their own terms and, if refused by any of them, commits no identity, so re-presenting it later is a first cancellation and not a replay. F712-AC-07's re-read of the acting actor's current platform authority still applies to the replay on that criterion's terms and is not replaced by the identity.
...
- F712-AC-03 includes, for a `queued` and for a `running` run alike, a fixture in which the cancellation commits, its response is lost, and the retry presenting the same cancellation identity returns the outcome that cancellation recorded under R-08 rather than being rejected as already terminal under R-07, cancelling nothing a second time, plus a mismatched-reuse fixture in which that committed identity is re-presented naming a different run and is refused as a conflict, cancelling nothing.
