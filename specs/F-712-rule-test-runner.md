...
3. **F712-AC-03:** Failure, evaluation error, timeout, cancellation, missing fixture, or changed input cannot count as a passing publication gate.
   ...
   A run reaches exactly one terminal state. The transitions out of `running` are exactly the terminal states the state prose names, passed, failed, errored, and cancelled; a timeout terminates the run in one of them and never leaves it in none. Every one of those transitions, cancellation included, names the run state and version it read and compare-and-swaps it inside the transaction that writes the terminal result. Exactly one therefore wins: a worker that finishes after a cancellation committed reads the cancelled state, rejects its write, and records no result, and a cancellation that arrives after a terminal result committed reads that result, rejects, and overwrites nothing. The losing side reports the terminal state that won rather than its own outcome, and the queued → running transition compare-and-swaps the same value so one worker claims a run.
