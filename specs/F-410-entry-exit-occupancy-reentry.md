...
**Phase:** 3 · **Issue:** [#42](https://github.com/jzeng151/pop-engine/issues/42) · **Owner:** TBD · **Reviewer:** product owner · **Approval date:** —
...
3. **F410-AC-03:** An invalid direction produces a visible conflict and no silent history rewrite. An authorized correction binds a stable client-supplied request identity, committed with the appended event under a uniqueness constraint scoped to the attendee's direction record within the event, to the exact unresolved operation set and expected attendee-direction version, compare-and-swaps both, and appends one auditable event; the scope is stated because F410-AC-02's validation identity is scoped to the event together with its own outcome and does not recognise a correction, and an identity keyed globally or per actor would answer a correction on one attendee with the outcome recorded for another; a recognized retry returns that result only when the operation set and version it presents equal the ones committed with that identity, per the operand-binding rule in F411-AC-08, while a mismatch is refused as a conflict, appends nothing, and requires rebuilt resolution. The correction's authorization is the acting actor's current authority, re-read server-side inside the correcting transaction under the terms F410-AC-08 states.
...
7. **F410-AC-07:** Every directional operation, online and offline alike, carries a client-generated operation identity supplied by the device, committed under a uniqueness constraint scoped to the event together with the operation's accepted or rejected outcome. A replay presenting the same identity returns that stored outcome and neither changes occupancy nor appends a second event; a genuinely separate scan sends a new identity. The acting actor's current authority is not one of the checks that identity is resolved past: the authority that admits a first attempt, membership and permission where an organizer acts and the equivalent admission where the caller is a public or door client, is re-read server-side at the replay, and the stored outcome is returned only if it still admits the request, so a replay presented after that authority is gone is refused and discloses no more than a first request would; the only exception is authority the committed operation itself removed, which `F411-AC-08` states once for this branch. This is operation identity, never content uniqueness: the same attendee legitimately enters, exits, and re-enters with otherwise identical payloads, so attendee, direction, and timestamp may not serve as the key, and a repeated identity is never rejected as a duplicate value.
...
8. **F410-AC-08:** Every directional operation names the event it acts in and is admitted only by the actor's current F-703 door permission for that exact event, re-read server-side from stored role and membership at the moment of the write and compared inside the same transaction that compare-and-swaps the attendee's directional state under F410-AC-05. A request failing that check is refused before any durable write: it records no directional event, changes no occupancy, commits no F410-AC-07 identity row, and discloses nothing about whether the attendee or the event exists. The check is at the write and not at session start, so a permission removed while an operation is in flight causes that operation to fail rather than commit. Corrections under F410-AC-03 already require authorization and keep the permission they name; this criterion adds the ordinary entry and exit path beside them rather than replacing it.
   ...
   The reads this feature defines are admitted on the same terms. Reading the derived occupancy projection and reading the append-only entry/exit/correction history each name the event, and each is admitted only by the actor's current F-702 membership of the owning workspace together with the F-703 permission approved for that read, re-read server-side from stored role and membership at the moment of the read. A read failing the check is refused before any count, directional event, correction, or attendee state is disclosed, and its response does not distinguish an event or attendee that does not exist from one the actor may not see. Neither read had a gate before this paragraph: the directional writes and the corrections were checked while the projection those writes exist to produce, and the history that audits them, were readable by any caller who could name the event. Until F-703 names the read permission, this leg is testable only at the same membership-level fallback stated above.

   **The same boundary binds the directional subject an operation acts on, not only the event it names.** The attendee, check-in, or credential a directional operation names, the attendee-direction record F410-AC-05 compare-and-swaps, and every operation in the unresolved set a F410-AC-03 correction resolves, must each resolve to the event this criterion admits and to the workspace that owns it, compared server-side inside the same transaction that accepts the direction or commits the correction, and again on every read that returns an occupancy count, a directional event, or a correction. A mismatch on either scope refuses the operation in whole: it records no directional event, changes no occupancy, commits no F410-AC-07 identity row, and its refusal does not distinguish an attendee, check-in, or credential that does not exist from one belonging to an event the actor may not act in.

   Admitting the event is not the same as placing the subject in it. Door staff working two events of one workspace hold the door permission for both, so the check above passes for event B while the scan names an attendee, check-in, or credential of event A. F410-AC-07 binds the supplied event and attendee into the replay identity's operands, which decides whether two requests are the same operation and never whether the attendee is one of B's; F410-AC-05 compare-and-swaps that attendee's own directional state, which is current wherever it lives; and F410-AC-03's correction identity is scoped to the attendee's direction record within the event, which likewise records where the correction landed rather than whose attendee it moved. A's direction is therefore appended to B's history and counted in B's occupancy while every stated check passes, and because F410-AC-04 supplies that count as the precondition for F-402's occupancy language, the corrupted figure is published as an operational claim about B rather than kept as a private inconsistency. It is invisible from A, where an attendee being scanned elsewhere changes nothing. Nothing above closes it: F411-AC-03 denies a wrong-event credential at the credential leg alone, and this feature accepts an attendee or check-in directly.

   The event a credential, check-in, and attendee-direction record resolves to is an approved input today, so the event comparison is implementable and testable now as "a directional operation or correction whose named attendee, check-in, or credential does not resolve to the event the operation acts in is refused, changes no occupancy, appends no event, and discloses neither side". F-702 is `PROPOSED`, so the workspace a record resolves to is not an approved input today and that half is not implementable until F-702 is approved and listed in `docs/BASELINE.md`.

## Fixtures and Verification

- Planned automated fixture IDs are the acceptance IDs above; each must map one-to-one to a runnable test before approval can claim implementation readiness.
- F410-AC-08 includes a fixture in which a same-workspace member without the door permission submits an entry and is refused with no directional event, no occupancy change, and no identity row, and one in which the permission is removed while an operation is in flight and that operation fails rather than commits. It also includes a fixture in which an actor holding no membership of the owning workspace names a valid event and is refused at the occupancy projection read and at the entry/exit/correction history read, with a response that does not distinguish absence from denial. It further includes a fixture in which door staff holding the door permission for both of two events submit, against event B, a direction naming an attendee, check-in, or credential of event A, and one in which a correction under F410-AC-03 names an unresolved operation from event A: each is refused, leaves both events' occupancy and history untouched, and returns a response that does not distinguish absence from denial.
...
- Approve directional state machine, correction policy, attendee correlation, and occupancy coverage wording. The approval must also name, with F-703, the door permission F410-AC-08 requires for an ordinary entry or exit and the occupancy and history read permission its read paragraph checks, because that criterion checks permissions no approved artifact defines today and may not invent them; until the matrix names them they are testable only at the membership level stated there.
