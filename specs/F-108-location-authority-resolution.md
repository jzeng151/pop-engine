...
**Phase:** 4 · **Issue:** [#53](https://github.com/jzeng151/pop-engine/issues/53) · **Owner:** TBD · **Reviewer:** product owner · **Approval date:** —
...
6. **F108-AC-06:** Starting a lookup binds the request to a stable client-supplied request identity, committed with the proposal under a uniqueness constraint scoped to the Event. A retry presenting the same identity returns the original proposal and issues no second provider call; a deliberate re-resolution of the same location sends a new identity. This is request identity, never content uniqueness: two genuinely distinct lookups of the same address are both recorded, and a repeated identity is never rejected as a duplicate value. That identity binds its operands under the rule `F411-AC-08` states once for every client-supplied identity on this branch: it is committed together with the request fields that determine the recorded result, including every aggregate, version, and generation the request names, and a later request presenting the same identity with any different operand is refused as a conflict rather than being answered with the stored result. The identity is still the key and the operands are only a precondition on reusing it, so this is not the content uniqueness this criterion already forbids. That identity is resolved in the order `F411-AC-08` states once for every client-supplied identity on this branch, applied here rather than restated: a request presenting an already committed identity is resolved from that record before the version, generation, state, and limit checks this criterion requires, and returns the outcome that request originally recorded, because the commit the retry repeats is exactly what moved the state those checks read. The acting actor's current authority is not among the checks the identity is resolved past: the authority this feature requires for a first attempt is re-read server-side at the replay, and the stored outcome is returned only if it still admits the operation, so a replay presented after that authority is gone is refused and discloses no more than a first request would; the only exception is authority the committed operation itself removed, which `F411-AC-08` states once for this branch. A request whose identity resolves to no committed outcome is then held to every one of them, and a request refused by one of them commits no identity, so re-presenting it later is a first attempt and not a replay.
   ...
   That request identity covers lookup starts and the proposal each one creates; it is not the record key for the rest of the aggregate. Correction and manual resolution under AC-03 and confirmation under AC-05 never create a proposal under this criterion's uniqueness constraint and do not consume or reuse a lookup identity. Each of them instead carries its own stable client-supplied request identity, distinct from the lookup identity and from every other correction's or confirmation's: a correction or manual resolution commits its identity with the corrected proposal version it appends, under a uniqueness constraint scoped to the Event, and a confirmation commits its identity through F-107's ordinary revision save as the save identity `F107-AC-07` defines, committed with the revision and the pointer and staleness result that save produces. Each identity is resolved in the order `F411-AC-08` states once for every client-supplied identity on this branch: a request presenting an already committed identity is resolved from that record before the proposal-version comparison AC-03 requires and before the base-revision, proposal-version, and selected-reference comparisons AC-05 requires, and returns the outcome that request originally recorded. Each binds its operands under the same rule, so a reuse naming a different selected reference, proposal version, or base revision is refused as a conflict rather than answered with the stored outcome, and a deliberate second correction or confirmation sends a new identity and is compared normally.

   The compare-and-swap alone cannot stand in for this. When a correction or a confirmation commits and its response is lost, the retry still names the proposal version or base revision the organizer read before that commit, which the commit itself advanced, so compared first it is stale by construction and the organizer is told their correction or confirmation conflicted when it succeeded. Returning the current proposal is not the same as returning the committed operation's outcome, and for confirmation the outcome is an appended Event revision and an advanced current-revision pointer, not a proposal at all. Resolving the identity first returns the result that already succeeded and leaves each comparison to reject only a genuinely stale second operation composed from an older observed state. The three therefore compose without any one owning the record: AC-06 decides how many proposals one intended lookup may create, each operation's own identity decides how many times one intended correction or confirmation may commit, and AC-03 and AC-05 decide which proposal version and which base revision a correction or confirmation is allowed to act on.
...
- F108-AC-06 includes a fixture in which a correction commits, its response is lost, and the retry presenting the same correction identity and the pre-correction proposal version returns the original corrected proposal version rather than a stale-version rejection, appending no second version; the same fixture for a confirmation, in which the retry presenting the same save identity and the pre-confirmation `base_revision_id` returns the original revision and pointer state rather than `revision_conflict`; and a mismatched-reuse fixture for each in which the committed identity is re-presented with a different selected authoritative reference and is refused as a conflict, mutating nothing.
