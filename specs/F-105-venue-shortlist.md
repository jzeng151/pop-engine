...
- F-101, the F-107 immutable revision lifecycle defined by `docs/EVENT-REVISION-CONTRACT.md`, and the F-701/F-702/F-703 gate. F-702 supplies the workspace membership boundary a venue candidate and its target Event resolve against and F-703 supplies the permission matrix `F105-AC-07` checks; F-701 supplies the authenticated actor both read from. All three are PROPOSED, so the gate is not an approved input today and this spec is not implementable against it until they are approved and listed in `docs/BASELINE.md`.
...
7. **F105-AC-07:** Every operation this feature defines names the venue candidate or Event it acts in and the workspace that owns it, and is admitted only by the acting actor's current F-702 membership of that workspace together with the F-703 permission approved for the action, both re-read server-side from stored membership and role at the moment of the operation and, for a write, inside the same transaction that commits it. That covers creating, updating, and archiving a candidate under F105-AC-06, listing and reading candidates under F105-AC-01, comparing candidates under the Scope's first In-scope line, building a prefill preview under F105-AC-02, and applying a prefill under F105-AC-02 and F105-AC-04. The apply is a cross-aggregate write: it reads a candidate and commits an F-107 revision on the target Event, so the transaction that appends that revision re-reads the actor's membership and permission for the Event it writes, not only for the candidate it read. This criterion composes with F105-AC-01 and does not relax it: F105-AC-01's creator-private boundary remains the rule inside the workspace, so a candidate operation is admitted only by this gate and by the acting actor being the creating user, and a workspace member who is not the creator is still refused. A request failing the check is refused before any durable write and before any candidate fact, note, tag, or Event value is disclosed, and its response does not distinguish a candidate or Event that does not exist from one the actor may not see. The check is at the operation and not at session start or workspace switch, so authority removed while a request is in flight causes that request to fail rather than commit.

   Without this criterion F105-AC-01 through F105-AC-06 all pass for a caller who names another workspace's candidate or Event. F105-AC-01 names the workspace authorization boundary and the creating user but never states what establishes that boundary, when it is read, or that it is re-read at the operation, and the others fix version comparison, field mapping, revision append, marketplace claims, and request identity without asking whether the actor still holds any authority at all. The surface that set leaves open reads another user's candidate names, private locations, notes, and tags, and the apply path commits an immutable revision to an Event in a workspace the actor was removed from.

   One input this criterion needs is not established by any approved artifact today and is not invented here. F-703 is PROPOSED and names no role set, so the permission above cannot be named. Until F-703 is approved this criterion is testable only as "every candidate create, update, archive, list, read, and compare, and every prefill preview and apply, is refused unless the acting actor holds an active membership of the workspace that owns the named candidate or Event, read server-side at that operation and, for a candidate operation, is the creating user F105-AC-01 names, and a refusal discloses nothing about whether that candidate or Event exists", not against a named role or permission identifier. Naming the candidate management and prefill permissions with F-703 is an approval blocker below.

8. **F105-AC-08:** Every operation this feature defines that names more than one record requires all of the records it names to resolve to the same workspace, checked server-side at the operation and, for a write, inside the transaction that commits it. The prefill preview and apply under F105-AC-02 and F105-AC-04 name a venue candidate and a target Event separately, and F105-AC-07 admits them by the actor's current membership of the candidate's workspace and, for the apply, of the Event's workspace. Those two checks are independent, so a user who holds a membership of workspace A and a membership of workspace B satisfies both while naming a candidate in A and an Event in B: the actor is legitimately authorized in each place they stand, and the pinned candidate version genuinely exists. A mismatch is refused before the preview is built and before any revision is appended, mutates nothing, discloses no candidate or Event fact, and returns a response that does not distinguish a record that does not exist from one that resolves to a different workspace. `F502-AC-08` states this rule for the several events one comparison names and `F503-AC-07` states it for a template and the Event created from it; this is the same rule applied to the two records this feature names together, and it is stated here because an implementation is built to this spec's criteria.

   Without this criterion F105-AC-01 through F105-AC-07 all pass for that caller. F105-AC-01 makes a candidate private to its creating user inside its workspace and the user naming it is the creator, F105-AC-02 compare-and-swaps a candidate version and a `base_revision_id` that are both current, F105-AC-03 through F105-AC-05 govern unanswered facts, the revision append, and marketplace claims, F105-AC-06 recognises the request identity, and F105-AC-07 finds a real membership behind each half of the operation. The preview reads workspace A's private candidate name, location facts, notes, and tags into a workspace B surface, and the apply commits them into an immutable B revision that keeps the venue source link the Scope's third In-scope line requires, so B's members read A's private venue research from that revision afterwards and A's organizer sees a candidate that was only ever read by its own creator. This is the one direction the actor check cannot see, because the actor did nothing they were not entitled to do in either workspace separately.

   No approved artifact establishes a cross-workspace venue sharing rule and this criterion does not invent one. Sharing candidates between workspaces would be a Scope change returning to the PRD and Roadmap decision process under `docs/DOCUMENTATION-GOVERNANCE.md` §7, and it would additionally need F-702 to define what a shared candidate is and F-703 to name the permission admitting the share. F-702 is PROPOSED, so the workspace a record resolves to is not an approved input today and this criterion is not implementable until F-702 is approved and listed in `docs/BASELINE.md`; until then it is testable only as "a prefill preview or apply naming a candidate and a target Event whose owning workspaces differ is refused, appends no revision, and discloses neither record", not against a named workspace model.
...
- F105-AC-08 includes a fixture in which an actor holding an active membership of both workspaces names a candidate owned by them in one workspace and a target Event in the other and is refused at prefill preview and at apply, with no revision appended and a response that does not distinguish absence from a different workspace, and a same-workspace control fixture in which the identical preview and apply succeed.
- F105-AC-07 includes a fixture in which an actor holding no membership of the owning workspace names a valid candidate and target Event and is refused at create, update, archive, list, compare, prefill preview, and apply, with a response that does not distinguish absence from denial, a fixture in which a workspace member who is not the creating user is refused every candidate operation under F105-AC-01, and a fixture in which membership removed while an apply is in flight fails the request rather than committing the revision.
...
- Approve F-701, F-702, and F-703, and name with F-703 the candidate management and prefill permissions `F105-AC-07` checks. That criterion checks a permission no approved artifact defines today and may not invent one, so until the matrix names them it is testable only at the membership level stated there.
- Approve F-702 so that the workspace a venue candidate and a target Event each resolve to is an approved input `F105-AC-08` can compare. That criterion compares a boundary no approved artifact defines today and may not invent one.
