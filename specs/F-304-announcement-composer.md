...
**Phase:** 2 · **Issue:** [#21](https://github.com/jzeng151/pop-engine/issues/21) · **Owner:** TBD · **Reviewer:** product owner · **Approval date:** —
...
- F-101 confirmed event data and the F-701/F-702/F-703 gate. F-702 supplies the workspace membership boundary the event and its AI runs and drafts resolve against and F-703 supplies the permission matrix `F304-AC-07` checks; F-701 supplies the authenticated actor both read from. F-701 is APPROVED (2026-07-28, `docs/BASELINE.md`); F-702 and F-703 remain PROPOSED, so the gate is not an approved input today and this spec is not implementable against it until those two are approved and listed in `docs/BASELINE.md`.
...
7. **F304-AC-07:** Every operation this feature defines names the event it acts in and the workspace that owns it, and is admitted only by the acting actor's current F-702 membership of that workspace together with the F-703 permission approved for the action, both re-read server-side from stored membership and role at the moment of the operation and, for a write, inside the same transaction that commits it. That covers starting a generation and its AI run create under F304-AC-06, every run status and result read, draft retrieval and the source-fact preview under F304-AC-01 and F304-AC-02, copying, editing, and regenerating a draft and the pre-copy recheck under F304-AC-01 and F304-AC-03, and the explicit organizer reconciliation F304-AC-01 defines. A request failing the check is refused before any durable write and before any confirmed event fact, source-fact preview, draft, or run result is disclosed, and a refused generation issues no provider call, so a caller the check turns away sends nothing of the event to the provider either. Its response does not distinguish an event or run that does not exist from one the actor may not see. The check is at the operation and not at session start or workspace switch, so authority removed while a request is in flight causes that request to fail rather than commit.
   ...
   **The same boundary binds the facts a draft is composed from, not only the event it names.** Every selected fact and fact version pinned under F304-AC-01, and every concrete event claim bound to a confirmed source fact under F304-AC-04, must itself resolve to the event being advertised and to the workspace that owns it. The set is checked at selection and re-checked at preview, at generation, before the provider receives the approved public fields under F304-AC-02, and before every copy and organizer reconciliation, on the same terms the version recheck in F304-AC-01 already runs on. A draft naming a fact outside that set is refused: it composes nothing, sends nothing to the provider, and its refusal does not distinguish a fact that does not exist from one belonging to an event the actor may not advertise from.

   Pinning a fact's version is not the same as placing it in this event. An actor who can see two events passes F304-AC-01's pinning and F304-AC-04's source-fact match by selecting event A's public facts into a draft for event B, because each check asks whether the fact is confirmed and current rather than whose it is. The result is an announcement that is copyable, provider-generated, and misattributed to B, which is the misstatement F304-AC-04 exists to prevent, reached through the one route that criterion does not inspect.
...
- F304-AC-07 includes a fixture in which an actor holding no membership of the owning workspace names a valid event and run and is refused at generation start, at every run and draft read, at preview, and at copy, with a response that does not distinguish absence from denial and with no provider call issued, and a fixture in which authority removed while a generation request is in flight fails that request rather than committing a run. It also includes a cross-event fixture in which an actor who can see two events composes a draft for event B from a confirmed, version-pinned event-A public fact: the composition is refused, no provider call is issued, and no draft becomes copyable or reconcilable.
...
- Approve F-702 and F-703, and name with F-703 the generation and draft permissions `F304-AC-07` checks. That criterion checks a permission no approved artifact defines today and may not invent one, so until the matrix names them it is testable only at the membership level stated there.
