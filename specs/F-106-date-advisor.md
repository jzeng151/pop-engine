...
- Resolve OPEN-QUESTIONS R-10 and approve the date/holiday ADR. SPEC-CONFLICT #130 is resolved for F-201 by requiring production to return NOT_CALCULABLE while no holiday list is published; it did not publish the calendar F106-AC-03 needs to rank candidate dates on business-day windows. The remaining calendar-source and publication work, not #130 itself, blocks this proposed feature.
- Approve evaluation request limits and result wording.
- Approve F-702 and F-703, and name with F-703 the advisor read and apply permissions `F106-AC-06` checks. That criterion checks a permission no approved artifact defines today and may not invent one, so until the matrix names them it is testable only at the membership level stated there.
