...
**Status:** APPROVED (2026-07-24; Outputs amended 2026-08-02, product-owner approved, one person currently holding every lane: the three rescope enrichment fields the engine already emits are written down, and `verdict_detail` prose is pinned to rule ids, with the render-time substitution stated as what it actually delivers and its one merged-finding residue named. No verdict, finding, deadline, threshold or regulatory fact moves) · **Reviewer/approver:** product owner · **Owner:** see Lane below · see `docs/BASELINE.md`.
