...
**Updated:** 2026-07-22 against nyc.v2.1; retargeted through nyc.v2.8 for the changes recorded in `docs/BASELINE.md`, to nyc.v2.9 on 2026-07-29, to nyc.v2.10 the same day for issue #181's citation-only correction, to nyc.v2.11 for organizer summaries, to nyc.v2.12 on 2026-08-12 for issues #258 and #287, and to nyc.v2.13 the same day for issue #144's assertion-free coverage-advisory publication. v2.9 adds `no_new_requirement` confirmation findings and changes the active intake registry for F-110 and issue #194 without changing a verdict. v2.10 and v2.11 change no finding, status, or verdict. v2.12 changes existing structure and fuel findings under the approved overall-prohibition verdict semantics. The plan renderer's separate near-empty predicate is disposition-based: a `required + not_calculable` finding remains definite and suppresses near-empty copy.
...
**Reachability, measured rather than assumed.** On `rules/nyc-rules.v2.13.json` no `advisory` or `no_new_requirement` rule publishes a deadline, so branch 3 is unreachable on the published ruleset and arises on the proposed draft and in fixtures. Branch 4 is reachable today. That bounds the harm of the sentences the three earlier branches produced for those shapes; it does not make them true, which is why they are amended rather than left.
