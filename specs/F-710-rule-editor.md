...
- F-711 sources, F-703 separate rules-admin role, and approved schema/type authority. F-703's role is what admits every operation `F710-AC-08` names, on the terms `F703-AC-04` states, and F-701 supplies the authenticated actor that check reads. F-701 is APPROVED (2026-07-28, `docs/BASELINE.md`); F-703 remains PROPOSED, so the gate is not an approved input today and this spec is not implementable against it until F-703 is approved and listed in `docs/BASELINE.md`.
...
- Approve F-703, whose role/action matrix and platform-role administration path are what make `F710-AC-08`, and the acting rules administrator already named in F710-AC-07's identity scope, testable against a named role rather than against the shape of a check.
