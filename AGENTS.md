...
5. `rules/nyc-rules.v2.12.json` and `docs/test-scenario-answer-key.md` when the feature touches rules, plans, or verdicts
...
- Regulatory output comes only from the published ruleset (`rules/nyc-rules.v2.12.json`). Authority order: approved primary source → published rule → approved fixture → engine output → UI copy. When levels disagree, fix the lower one; never bend the engine to reproduce an unsupported expectation.
